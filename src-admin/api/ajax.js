...
axios.interceptors.request.use(function (config) {
    ...
    if(method.toLowerCase() === 'post' && typeof data === 'object'){
       config.data = qs.stringify(data)//转换之后的字符串也要重新赋值，username=admin&password=admin

    }

    return config;//接收是config,返回的也是config
  })
...
// 添加响应拦截器
//功能1：让请求成功的结果不再是response，而且response.data的值
//功能2：统一处理所有请求的异常错误
